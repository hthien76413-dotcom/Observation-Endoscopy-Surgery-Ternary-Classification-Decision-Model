...
| set | 含义 | 宏平均 AUC | 能否入稿 |
|---|---|---|---|
| `apparent` | 模型在自己的训练数据上预测 | 0.826 | **否**，必然乐观 |
| `cv` | 折内插补的 5 折交叉验证 | **0.792** | **是，Fig 4 A 栏用这套** |
| `temporal` | 2023 年后的独立时段 | 0.772 | 是，Fig 4 B 栏 |

表观与交叉验证差 0.034。若 Fig 4 画表观而 Table 3 报交叉验证，图与表在同一篇
...
**这张图的要点是两条线的分离。** 判别力经受住时间验证（宏平均 0.792 → 0.772），
校准没有：

| | 观察 | 内镜 | 手术 |
|---|---|---|---|
| 交叉验证 斜率/截距 | 0.93 / −0.01 | 0.97 / −0.00 | 1.15 / +0.05 |
| 时间验证 斜率/截距 | 0.75 / **+0.46** | 0.90 / **−0.32** | 1.33 / **−0.57** |
| 构成比变化 | 47.1% → 55.9% | 42.0% → 36.3% | 10.9% → 7.7% |

截距漂移方向与构成比变化一致——截距正是承接患病率的那一项。结论是模型若要用于
后续年份，须先重估截距。只报 AUC 0.772 会让读者以为时间验证通过即可直接部署。

两处实现细节：

- **分箱数随样本量伸缩**（`n // 120`，限 4–10 箱）。时间验证 n=413、手术仅 32 例，
  硬切 10 箱会有半数箱子实际比例为 0，画出来是锯齿而不是校准。
- **DCA 阴影标出模型优于两条参照线的区间**（观察 0.21–0.75、内镜 0.07–0.74、
  手术 0.02–0.80），由脚本实算而非写死。低于该区间时 treat-all 更优，是阈值低于
