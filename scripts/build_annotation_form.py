...
FIELDS = [
    ...
    ("最大径_mm", True, None, 11, "最大一枚异物的最大径，毫米，整数"),
...
def build_instructions(ws, n_rows, counts):
    ...
    tips = [
        ...
        "5.  「异物可见 = 否」时，部位填「无异物」，数目填「不能确定」，最大径填 0，成串成团填「不适用」。",
        "6.  双人阅片行由两人各填一份独立文件，切勿互相参照，最后再合并算 Kappa。",
        "7.  拿不准就填「不确定」／「不能确定」，并在备注里写一句。不要猜。",
    ]
    r = 22
    for t in tips:
    ...
    ex_head = ["阅片者", "阅片日期", "异物可见", "部位", "数目", "数目_精确",
               "最大径_mm", "成串成团", "形态", "双环征", "影像判断类型", "备注"]
    ex_val = ["ZS", "2026-09-20", "是", "胃", "3-5", "4", "5", "是", "球形", "不适用",
              "磁性球形", "四枚成串吸附，位于胃体"]
    hr = r + 2
...
def build_codebook(ws):
    ...
    defs = {
        ...
        "最大径_mm": "最大一枚异物的最大径。请用 PACS 测量工具实测，不要目测估计；"
                     "指南阈值（直径 >25 mm 或长度 >60 mm 难过幽门）依赖这一项。",
